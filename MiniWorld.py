...
def insertInAccount():
    ...
    try:
        ...
        row = {}
        print("Enter new customers details: ")
...
def changeridestate():
   try:
       ...
       query = "UPDATE Ride SET status = {} WHERE rideno={};".format(
           k[1], k[0])
       print(query)
       cur.execute(query)
       con.commit()
       ...
       print("Failed to insert into database")
       print(">>>>>>>>>>>>>", e)
def changedriverpassword():
   try:
       k = input("driver ID: ")
       t = input("newpassword: ")
       query = "UPDATE Account SET password={} WHERE id={}".format(t, k)
       print(query)
       cur.execute(query)
       con.commit()
       print("inserted into database")
       ...
       print("Failed to insert into database")
       print(">>>>>>>>>>>>",e)
def updatedriverphnno():
   try:
       k = input("driver ID: ")
       t = input("newphone no: ")
       query = "UPDATE Contact SET Contact_number={} WHERE id_account={}".format(t, k)
       print(query)
       cur.execute(query)
       con.commit()
       print("inserted into database")
...
def Hire_a_driver():
    ...
    try:
        ...
        row["Driver_Id"] = int(input("Driver_Id see from Account Table: "))
...
def dispatch(ch):
    ...
    if(ch == 1):
        ...
        password = input("Password: ")
        try:
            query = "SELECT username from Account WHERE username = {} and  Password = {};".format(username,password);
            # gets the number of rows affected by the command executed 
            print(query)
            cur.execute(query)
            con.commit()
            row_count = pymysql.cursor.rowcount
            if row_count ==0 : print ("Wrong password or UserName")
            else:
                rows = cur.fetchall()
                Driver_id = rows[0]["Id"]
                while(1):
                    ...
                    print("1. changeridestate")  
                    print("2. Fire a driver")
                    print("3. Remove not in service vehicles")  
